    ...
    <div className="min-h-screen w-full bg-slate-950 text-white flex flex-col font-sans selection:bg-blue-500/30">
      ...
      <header className="w-full py-6 px-8 border-b border-slate-800 bg-slate-950/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          <button onClick={() => setView("quiz")} className="flex items-center gap-2 cursor-pointer group bg-transparent border-none p-0">
      ...
      <main className="flex-grow flex items-center justify-center p-6 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-slate-950">
        {view === "quiz" && (
          <div className="w-full max-w-xl bg-slate-900/80 border border-slate-800 p-8 rounded-3xl shadow-2xl backdrop-blur-sm">
            {!showForm && !showResult && (
            ...
            {showResult && (
              <div className="text-center py-6">
                ...
                <button onClick={resetQuiz} className="text-slate-600 hover:text-white font-bold uppercase tracking-widest text-[10px]">Restart Discovery</button>
              </div>
            )}
          </div>
        )}
        ...
        {view === "portfolio" && (
          <div className="w-full max-w-5xl text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <a href="https://game-day-tracker.vercel.app/" target="_blank" className="bg-slate-900 border border-slate-800 rounded-3xl p-12 hover:border-blue-500 transition-all">Game Day Tracker</a>
            </div>
          </div>
        )}
      </main>
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-start gap-12">
          <div className="max-w-sm">
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="font-bold tracking-tight text-sm uppercase italic">Shinn <span className="text-blue-500">Digital</span></span>
            </div>
          ...
          <div className="flex flex-col md:items-end gap-6 text-sm">
            <div className="flex gap-8">
              <a href="https://www.linkedin.com/in/david-shinn1/" target="_blank" className="text-slate-400 hover:text-blue-400 transition-colors uppercase font-bold text-xs">LinkedIn</a>
              <a href="mailto:david@shinndigital.com" className="text-slate-400 hover:text-blue-400 transition-colors uppercase font-bold text-xs">Email</a>
            </div>
